...
export class OffersScreen extends React.Component{
  ...
  async componentDidUpdate(e){
    ...
    if(e.productsData !== this.props.productsData){
      let productsData = this.props.productsData.filter(data=>data.status==='Active');
      let id = this.state.AllData.id;
      if(id==='MyOffers'){
        productsData = productsData.filter(data=>parseInt(data.seller)===getState().auth.user.id);
      }else if(id==='featured'){
        productsData = productsData.filter(data=>data.featured===true);
      }else if(id==='last'){
          productsData = productsData.sort(function(a,b){
            return new Date(a.createdAt) - new Date(b.createdAt);
          });
          productsData = productsData.length>5?productsData.slice(0, 5):productsData;
      }else{
        productsData = productsData.filter(data=>parseInt(data.category)===id);
      }
      let products = [];
    ...
    return(
      <Container style={styles.container}>
        <Headers 
          ...
          title={`${this.state.AllData.title} Offers`}
        ...
        <View style={styles.content}>
          ...
          <View style={[styles.itemList,{height:LAYOUT.window.height*0.8}]}>
            <ScrollView>
